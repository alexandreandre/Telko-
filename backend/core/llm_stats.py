"""
Agrégation des statistiques d'usage LLM pour le comparateur.
Stockage unique : table Supabase `llm_runs` (une ligne par exécution, notation mise à jour ensuite).

Lecture des agrégats : uniquement depuis `llm_runs` (pas de fichier local, pas de SQLite feedback).
"""
...
_MAX_USER_EXCERPT = 8000
_MAX_ASSISTANT_EXCERPT = 32000
...
def _excerpt_field(meta: Dict[str, Any], key: str, max_len: int) -> str | None:
    raw = meta.get(key)
...
def _row_payload_from_meta(meta: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    if run is None:
        logger.warning("record_llm_run — meta invalide (from_meta), insertion ignorée.")
        return None
    ...
    user_ex = _excerpt_field(meta, "user_prompt_excerpt", _MAX_USER_EXCERPT)
    asst_ex = _excerpt_field(meta, "assistant_response_excerpt", _MAX_ASSISTANT_EXCERPT)
    ...
    if user_ex is not None:
        payload["user_prompt_excerpt"] = user_ex
    if asst_ex is not None:
        payload["assistant_response_excerpt"] = asst_ex
...
def _insert_run_supabase(payload: Dict[str, Any]) -> None:
    ...
    if headers is None:
        logger.error(
            "record_llm_run — Supabase non configuré (supabase_url / clé manquante), run non enregistré."
        )
        return
...
def _iter_runs_supabase() -> List[LLMRun]:
    """Charge tous les runs depuis Supabase (source unique pour le comparateur)."""
    headers = _supabase_headers()
    if headers is None:
        logger.warning("build_usage_aggregates — Supabase non configuré, aucun run.")
        return []
...
def record_llm_run(meta: Dict[str, Any]) -> None:
    """
    Enregistre une exécution LLM dans Supabase uniquement.
...
def build_usage_aggregates(
    ...
    Source unique : lignes Supabase `llm_runs`.

    Retourne un tuple :
      - usage_rows : stats par modèle
      - global_stats : agrégats globaux
      - feedback_stats : satisfaction par modèle (runs notés uniquement)
    """
